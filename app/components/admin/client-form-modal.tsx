...
export function ClientFormModal({ client, isExisting, onClose }: ClientFormModalProps) {
  ...
  const hasUnsavedChanges = useMemo(() => {
    ...
    if (clientNumber.trim() !== client.clientNumber.trim()) {
      return true;
    }

    if (address.trim() !== client.address.trim()) {
      return true;
    }
  ...
  }, [query, clientNumber, address, attachedIds, client]);
  ...
  const hints = useMemo(() => {
    ...
    return state.meters.filter(
      (meter) =>
        !attachedIds.includes(meter.id) && meterMatchesQuery(meter, query),
    );
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    if (!trimmedNumber || !trimmedAddress) {
      return;
    }
    ...
    >
      ...
      >
        <div className="flex shrink-0 items-start justify-between gap-3 px-6 pt-6">
          <div>
            <h3 id="client-form-modal-title" className="text-base font-semibold text-zinc-900">
              {isExisting ? "Labot klientu" : "Jauns klients"}
            </h3>
            {isExisting ? (
              <p className="mt-1 text-sm text-zinc-500">
                Atjauniniet klienta datus un piesaistītos skaitītājus
              </p>
            ) : null}
          </div>
          <TooltipIconButton
          ...
          <div className="min-h-0 flex-1 space-y-5 overflow-y-auto px-6 pt-5">
            ...
            <div ref={inputWrapRef}>
              ...
              <IconInput
                ...
                aria-controls="client-meter-hints"
